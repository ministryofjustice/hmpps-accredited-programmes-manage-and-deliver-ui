...
describe('getNameSortValue', () => {
  ...
  const createPresenter = () => {
    ...
    const referralCaseListItemPage: Page<ReferralCaseListItem> = pageFactory
      .pageContent([referralCaseListItem])
      .build() as Page<ReferralCaseListItem>
    return new CaselistPresenter(1, referralCaseListItemPage, filter, '', true, caseListFilters, 0)
  }

  it('should format a full name as "surname, firstname" in lowercase', () => {
    const presenter = createPresenter()
    expect(presenter.getNameSortValue('Jane Smith')).toBe('smith, jane')
  })

  it('should handle names with multiple first names', () => {
    const presenter = createPresenter()
    expect(presenter.getNameSortValue('Jane Emma Smith')).toBe('smith, jane emma')
  })

  it('should handle single name as surname with no firstname', () => {
    const presenter = createPresenter()
    expect(presenter.getNameSortValue('Smith')).toBe('smith, ')
  })

  it('should return empty string when name is empty after trim', () => {
    const presenter = createPresenter()
    expect(presenter.getNameSortValue('')).toBe('')
    expect(presenter.getNameSortValue('   ')).toBe('')
  })
