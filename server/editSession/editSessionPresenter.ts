    ...
    if (this.hasMultipleReferrals) {
      return {
        ...
        rows: attendanceData.map((it, index) => ({
          ...
          cells: [
            {
              html: `<a href="/referral-details/${it.referralId}/personal-details">${it.name}</a> ${it.crn}${it.lao ? ' <br><span class="moj-badge moj-badge--red">RESTRICTED ACCESS</span>' : ''}`,
            },
      ...
      rows:
        attendanceData.length > 0
          ? [
              [
                {
                  html: `<a href="/referral-details/${attendanceData[0].referralId}/personal-details">${attendanceData[0].name}</a> ${attendanceData[0].crn}${attendanceData[0].lao ? ' <br><span class="moj-badge moj-badge--red">RESTRICTED ACCESS</span>' : ''}`,
                },
