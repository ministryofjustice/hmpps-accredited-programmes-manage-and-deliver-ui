...
describe('Edit Group Controller', () => {
  ...
  describe('GET /group/:groupId/edit-group-facilitators', () => {
    it('shows guidance that past sessions keep their assigned facilitators', async () => {
      accreditedProgrammesManageAndDeliverService.getGroupDetailsById.mockResolvedValue(
      ...
      accreditedProgrammesManageAndDeliverService.getPduMembers.mockResolvedValue([
        {
          personName: 'Treatment Manager',
          personCode: 'TM001',
          teamName: 'Team A',
          teamCode: 'TA001',
        },
        {
          personName: 'Facilitator 1',
          personCode: 'F001',
          teamName: 'Team A',
          teamCode: 'TA001',
        },
      ...
      return request(app)
        ...
        .expect(res => {
          expect(res.text).toContain(
            'The assigned facilitators will not change for any sessions that have already taken place.',
          )
        })
    })

    it('shows existing assigned staff as unavailable when not present in current PDU members list', async () => {
      accreditedProgrammesManageAndDeliverService.getGroupDetailsById.mockResolvedValue(
        GroupDetailsFactory.build({
          id: groupId,
          code: 'TEST123',
          treatmentManager: 'Legacy Person',
          facilitators: ['Active Facilitator'],
          coverFacilitators: [],
        }),
      )
      ...
      return request(app)
        ...
        .expect(200)
        .expect(res => {
          expect(res.text).toContain('Legacy Person (currently unavailable)')
        })
    })
  })

  describe('POST /group/:groupId/edit-group-facilitators', () => {
    it('preserves unavailable existing assigned staff when submitted unchanged', async () => {
      accreditedProgrammesManageAndDeliverService.getGroupDetailsById.mockResolvedValue(
        GroupDetailsFactory.build({
          id: groupId,
          code: 'TEST123',
      ...
      return request(app)
        ...
        .send({
          'create-group-treatment-manager': JSON.stringify({
            facilitator: 'Legacy Person',
            facilitatorCode: 'UNAVAILABLE:TREATMENT_MANAGER:Legacy Person',
            teamName: 'Unavailable',
            teamCode: 'UNAVAILABLE',
            teamMemberType: 'TREATMENT_MANAGER',
          }),
          'create-group-facilitator-existing-1': JSON.stringify({
            facilitator: 'Active Facilitator',
            facilitatorCode: 'AF001',
            teamName: 'Team A',
            teamCode: 'TA001',
        ...
        .expect(res => {
          ...
          expect(accreditedProgrammesManageAndDeliverService.updateSessionFacilitators).not.toHaveBeenCalled()
        })
    })
  })
})
