...
beforeEach(() => {
  const sessionData: Partial<SessionData> = {
    ...
    originPage: '/groupOverview/123/allocated?nameOrCRN=dave',
  }
  ...
  describe(`GET /removeFromGroup/:groupId/:referralId`, () => {
    ...
    it('posts to the remove from group page and redirects successfully to the allocated page with previous filters if No is selected', async () => {
      ...
      return request(app)
        ...
        .expect(302)
        .expect(res => {
          expect(res.text).toContain(`Redirecting to /groupOverview/${groupId}/allocated?nameOrCRN=dave`)
        })
    })
      ...
      return request(app)
        ...
        .expect(res => {
          expect(res.text).toContain(
            `Redirecting to /groupOverview/${groupId}/allocated?message=John%20Jones%20was%20removed%20from%20this%20group.%20Their%20referral%20status%20is%20now%20Suitable%20but%20not%20ready.`,
          )
        })
    })
    it('returns with errors if validation fails', async () => {
