  ...
  generateWaitlistTableArgs() {
    ...
    rows.forEach(member => {
      ...
      out.push([
        ...
        {
          html: `<a href="${this.referralHref(member.referralId)}">${member.personName}</a><p class="govuk-!-margin-bottom-0"> ${member.crn}</p>${member.isLimitedAccessOffender ? '<span class="moj-badge moj-badge--red">RESTRICTED ACCESS</span>' : ''}`,
        },
        {
          html: !isExcluded
